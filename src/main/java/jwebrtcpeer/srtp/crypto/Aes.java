...
public class Aes {
	...
	/**
	 * The simple name of the {@link CipherFactory} class/interface which is used as
	 * a class name suffix by the well-known {@link CipherFactory} implementations.
	 */
	...
	/**
	 * The {@link CipherFactory} implementations known to the {@link Aes} class
	 * among which the fastest is to be elected as {@link #fastestFactories} for
	 * each transformation.
	 */
	private static CipherFactory[] factories;

	/**
	 * The {@link CipherFactory} implementation which is (to be) used by the class
	 * {@link Aes} to initialize {@link Cipher}s.
	 */
	...
	/**
	 * The {@link Class}es of the well-known {@link CipherFactory} implementations.
	 */
	private static final Class<?>[] FACTORY_CLASSES = { SunJCECipherFactory.class, BouncyCastleCipherFactory.class,
			SunPKCS11CipherFactory.class, };

	/**
	 * The class to instantiate as a {@link CipherFactory} implementation to be used
	 * to initialized {@link Cipher}s.
	 *
	...
	/**
	 * The size of the data to be used for AES cipher benchmarks. This is chosen to
	 * be comparable in size to an SRTP packet.
	 */
	private static final int BENCHMARK_SIZE = 1250;

	/**
	 * The number of times to pre-execute the benchmark before executing it for
	 * real, to give the JVM time to run JITs and the like.
	 *
	...
	/**
	 * The input buffer to be used for the benchmarking of {@link #factories}. It
	 * consists of blocks and its length specifies the number of blocks to process
	 ...
	 * 
	 * @param name the name of the class, or null to let the implementation pick the
	 *             class to use.
	 */
	...
	private static class GcmBenchmark extends BenchmarkOperation {
		...
		public void run(Cipher cipher) throws Exception {
			/*
			 * Many GCM providers don't let us use two identical IVs in a row with the same
			 * key.
			 */
			iv[0] ^= 1;
	...
	/**
	 * Benchmarks a specific array/list of {@link CipherFactory} instances and
	 * returns the fastest-performing element.
	 *
	 ...
	 * @return the fastest-performing {@link CipherFactory} among the specified
	 *         {@code factories}
	 */
	private static CipherFactory benchmark(CipherFactory[] factories, int keySize, String transformation) {
		...
		for (int f = 0; f < factories.length; ++f) {
			...
			// (name) through setFactoryClassName(String), Practically, FACTORY_CLASS_NAME
			// may override
			...
			try {
				...
				if (cipher == null) {
					...
					/*
					 * If this is the user-chosen factory class, we don't really need to benchmark
					 * it; just verify that it's working.
					 */
					...
					log.append(getSimpleClassName(factory)).append(' ').append(time);
				}
			} catch (Throwable t) {
		...
		synchronized (Aes.class) {
			...
			if (factory == null) {
				try {
					...
					if (factory == null) {
						factory = Aes.fastestFactories.getOrDefault(transformation, DEFAULT_FACTORY);
					}

					CipherFactory oldFactory = Aes.fastestFactories.put(transformation, factory);
	...
	private static String getEffectiveFactoryClassName() {
		...
		// brevity and ease.
		if (Character.isUpperCase(factoryClassName.charAt(0)) && !factoryClassName.contains(".")
				&& !factoryClassName.endsWith(CIPHER_FACTORY_SIMPLE_CLASS_NAME)) {
			factoryClassName = Aes.class.getName() + "$" + factoryClassName + CIPHER_FACTORY_SIMPLE_CLASS_NAME;
		}
		return factoryClassName;
	}

	/**
	 * Initializes the {@link CipherFactory} instances to be benchmarked by the
	 * class {@link Aes} and among which the fastest-performing one is to be
	 * selected.
	 * 
	 * @return the {@link CipherFactory} instances to be benchmarked by the class
	 *         {@link Aes} and among which the fastest-performing one is to be
	 *         selected
	 */
	@SuppressWarnings("unchecked")
	private static CipherFactory[] createCipherFactories() {
		// The user may have specified a specific CipherFactory class
		// (name) through setFactoryClassName(String). Practically, the specified
		// FACTORY_CLASS_NAME
		...
		if (factoryClass == null) {
			...
			if (factoryClassName != null) {
				...
				for (Class<?> clazz : factoryClasses) {
					if ((clazz != null) && clazz.getName().equals(factoryClassName)
							&& CipherFactory.class.isAssignableFrom(clazz)) {
						Aes.factoryClass = factoryClass = (Class<? extends CipherFactory>) clazz;
						add = false;
						break;
					}
				}
	...
	/**
	 * Initializes {@link CipherFactory} instances of specific {@link Class}es.
	 *
	 * @param classes the runtime {@link Class}es to instantiate
	 * @return the {@link CipherFactory} instances initialized by the specified
	 *         {@code classes}
	 */
	private static CipherFactory[] createCipherFactories(Class<?>[] classes) {
	...
	/**
	 * Gets a {@link CipherFactory} instance to be used by the {@link Aes} class to
	 * initialize {@link Cipher}s.
	 *
	 * <p>
	 * Benchmarks the well-known {@link CipherFactory} implementations and returns
	 * the fastest one.
	 * </p>
	 * 
	 * @param transformation The transformation for which to get a factory.
	 *
	 * @return a {@link CipherFactory} instance to be used by the {@link Aes} class
	 *         to initialize {@link Cipher}s
	 */
	private static CipherFactory getCipherFactory(String transformation, boolean warmup) {
		CipherFactory[] factories = Aes.factories;
		/*
		 * Note: we use 128-bit keys to measure the performance -- we can't know the
		 * actual key length that will be used until init() is called. Presume for now
		 * that all implementations' performance is roughly proportional between key
		 * lengths.
		 */
	 ...
	 * {@link CipherFactory} to be used for display purposes of brevity and
	 * readability.
	 *
	 * @param factory the {@link CipherFactory} for which a simple class name is to
	 *                be returned
	...
	public static class SunPKCS11CipherFactory extends CipherFactory {
		...
		/**
		 * The indicator which determines whether {@link #provider} is to be used. If
		 * {@code true}, an attempt will be made to initialize a {@link Provider}
		 * instance. If the attempt fails, {@code false} will be assigned in order to
		 * not repeatedly attempt the initialization which is known to have failed.
		 */
		private static boolean useProvider = true;

		/**
		 * Gets the {@code java.security.Provider} instance (to be) employed for an
		 * (optimized) AES implementation.
		 *
		 * @return the {@code java.security.Provider} instance (to be) employed for an
		 *         (optimized) AES implementation
		 */
			...
			if ((provider == null) && useProvider) {
				try {
					...
					if (Provider.class.isAssignableFrom(clazz)) {
						...
						provider = (Provider) contructor.newInstance(
		 ...
		 *
		 * @throws Exception if anything goes wrong while initializing a new instance
		 */
