...
public class SRTPPolicy {
	...
	/**
	 * Construct a SrtpPolicy object based on given parameters. This class acts as a
	 * storage class, so all the parameters are passed in through this constructor.
	 *
	 ...
	 */
	public SRTPPolicy(int encType, int encKeyLength, int authType, int authKeyLength, int authTagLength,
			int saltKeyLength) {
	 ...
	 * retransmission to a peer that does not support the rtx payload.) <b>Note</b>:
	 * Never re-send a packet with a different payload!
	 *
	 * @param enabled {@code true} if send-side replay protection is to be enabled;
	 *                {@code false} if not.
	 */
	 ...
	 * <p>
	 * Turn this off if you need to be able to receive identical packets more than
	 * once (e.g., if you are an RTP translator, with peers that are doing
	 * retransmission without using the rtx payload.) <b>Note</b>: You must make
	 * sure your packet handling is idempotent!
	 *
